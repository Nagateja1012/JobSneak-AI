...
const stages = [
  ...
  { id: "stage-3", status: "not-started", tooltip: "Prompt AI", error: "" },
];
...
export function matchingHeart(value) {
  const fillPercentage = (value / 10) * 100;
  heartPercent.innerHTML = fillPercentage + "%";
  // Adjust height based on fillPercentage
  fillBackground.setAttribute("y", 24 - (24 * fillPercentage) / 100); // Start at bottom and move up
  fillBackground.setAttribute("height", (24 * fillPercentage) / 100); // Adjust height
}
