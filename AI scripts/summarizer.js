...
async function onContentChange(newContent) {
  const readsumary = (await chrome.storage.local.get("summary")).summary;
  if (readsumary) {
    ...
    if (newContent) {
      if (newContent.length > MAX_MODEL_CHARS) {
        updateWarning(
          `WARNING: Text is too long for summarization with ${newContent.length} characters (MAX ~4000 characters).`
        );
      } else {
        updateWarning("");
      }
...
async function createSummarizer(config, downloadProgressCallback) {
  if (!window.ai || !window.ai.summarizer) {
    updateStageColor(
      "stage-1",
      "error",
      "AI Summarization is not supported in this browser"
    );
  }
  SummarizerAiModel();
